...
MIN_TOP_K, MAX_TOP_K = 0, 127.0
MIN_REP_PENALTY, MAX_REP_PENALTY = 1.3, 1.6
MIN_MAX_NEW_TOKENS, MAX_MAX_NEW_TOKENS = 0, 700.0
MIN_SEQ_POS, MAX_SEQ_POS = 0, 4095.0
...
LOG_DIR_BASE = CUR_DIR / "logs_ddp_robust_final"
SAVE_DIR_BASE = CUR_DIR / "saved_models_ddp_robust_final"
...
# decode_params, EmbeddingDataset, EnhancedMLP, collate_fn 保持与上一版本（用户提供的那个版本）一致
# ... (此处省略这些类的代码，假设它们与您上一版本中经过优化的版本相同) ...
# 我将直接复制它们过来以保证完整性
def decode_params(encoded_param):
    encoded = int(encoded_param)
    temp_raw = ((encoded >> 24) & 0xFF) / 255.0
    temp = temp_raw * (MAX_TEMP - MIN_TEMP) + MIN_TEMP
    top_k_raw = (encoded >> 17) & 0x7F
    rep_pen_raw = ((encoded >> 9) & 0xFF) / 255.0
    rep_pen = rep_pen_raw * (MAX_REP_PENALTY - MIN_REP_PENALTY) + MIN_REP_PENALTY
    max_new_tokens_idx = (encoded >> 6) & 0x7
    max_new_tok = max_new_tokens_idx * 100.0
    return {
        'temperature': temp,
        'top_k': float(top_k_raw),
